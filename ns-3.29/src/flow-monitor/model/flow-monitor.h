...
public:
  ...
  void ReportFirstTx (Ptr<FlowProbe> probe, FlowId flowId, FlowPacketId packetId, uint16_t sliceNumber, uint32_t packetSize);
  /// FlowProbe implementations are supposed to call this method to
  ...
  typedef std::map<FlowId, FlowStats>::iterator FlowStatsContainerCI; //Substitui const_iterator por iterator
  /// Container: FlowProbe
  ...
  void SetExecutionTime(uint64_t value );
...
private:
  ...
  uint64_t executionTime; //Adicionei
