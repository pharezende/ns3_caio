...
FlowMonitor::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::FlowMonitor")
    ...
    .AddAttribute ("MaxPerHopDelay", ("The maximum per-hop delay that should be considered.  "
                   ...
                   TimeValue (Seconds (0.101)), //eh aqui que dropa os pacotes.
...
void
FlowMonitor::ReportFirstTx (Ptr<FlowProbe> probe, uint32_t flowId, uint32_t packetId, uint16_t sliceNumber, uint32_t packetSize)
{
  if (!m_enabled)
    {
      return;
...
FlowMonitor::ReportLastRx (Ptr<FlowProbe> probe, uint32_t flowId, uint32_t packetId, uint32_t packetSize)
{
  ...
  //#####Adicionei! Se o pacote chegar em um tempo maior que o limite, ele eh descartado!
  if(delay > m_maxPerHopDelay){
	  return;
  }
  ...
  for (TrackedPacketMap::iterator iter = m_trackedPackets.begin ();
       iter != m_trackedPackets.end (); )
    {
      ...
      if (now - iter->second.lastSeenTime >= maxDelay)
        {
    	  //std::cout << "TESTE:!!" << iter->second.lastSeenTime << "\n";
          ...
          flow->second.lostPackets++;//Comentei.
...
FlowMonitor::SerializeToXmlStream (std::ostream &os, int indent, bool enableHistograms, bool enableProbes)
{
  ...
  for (FlowStatsContainerCI flowI = m_flowStats.begin ();
       flowI != m_flowStats.end (); flowI++)
    {
	  ...
	  //####Adicionei
	  flowI->second.delayAVG = TimeStep (flowI->second.delaySUM.GetNanoSeconds() / flowI->second.rxPackets);
	  flowI->second.jitterAVG = TimeStep (flowI->second.jitterSUM.GetNanoSeconds() / flowI->second.rxPackets);
	  Time difference = Time::FromInteger (executionTime, Time::S) - flowI->second.timeFirstTxPacket;
	  flowI->second.throughput = ((8.0*flowI->second.rxBytes)/ 1000) / (1.0*difference.GetMilliSeconds() / 1000); //Kbits
	  //########
...
void
FlowMonitor::SetExecutionTime(uint64_t value ){
	executionTime = value;
}
