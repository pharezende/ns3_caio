...
bool
Ipv4FlowClassifier::Classify (const Ipv4Header &ipHeader, Ptr<const Packet> ipPayload, uint16_t* sliceNumber,
                              uint32_t *out_flowId, uint32_t *out_packetId)
{
  ...
  //#################################################
  //Retirar do trace o tráfego que vai dos UEs ao servidor.
  /*Ipv4Address ipv4 ("1.0.0.2");
  if(ipHeader.GetDestination().Get() == ipv4.Get()){
	  return false;
  }*/
  ...
  //Formatar como slices
  //#####FIELD###########

  if(dstPort >= 1000 && dstPort < 2000){ //Slice 1
	  tuple.sliceNumber = 1;
	  *sliceNumber = 1;
  }
  else if(srcPort >= 1000 && srcPort < 2000){ //Slice 11
	  tuple.sliceNumber = 11;
	  *sliceNumber = 11;
  }
  //#####################
  ...
  //#######AUDIO#############
  else if(dstPort >= 2000 && dstPort < 3000){ //Slice 2
	  tuple.sliceNumber = 2;
	  *sliceNumber = 2;
  }
  //#########################
  ...
  //########IMG FTP##########
  else if(dstPort >= 3000 && dstPort < 4000){ //Slice 3
	  tuple.sliceNumber = 3;
	  *sliceNumber = 3;
  }

  else if(srcPort >= 3000 && srcPort < 4000){ //Slice 13
	  tuple.sliceNumber = 13;
	  *sliceNumber = 13;
  }
  //#########################

  //#######TXT FTP###########
  else if(dstPort >= 4000 && dstPort < 5000){ //Slice 4
	  tuple.sliceNumber = 4;
	  *sliceNumber = 4;
  }

  else if(srcPort >= 4000 && srcPort < 5000){ //Slice 14
	  tuple.sliceNumber = 14;
	  *sliceNumber = 14;
  }
  //#########################
  ...
  else if(dstPort >= 12000 && dstPort < 13000){ //Slice 12 - Não é TCP
	  tuple.sliceNumber = 12;
	  *sliceNumber = 12;
  }
...
Ipv4FlowClassifier::SerializeToXmlStream (std::ostream &os, uint16_t indent) const
{
  ...
  for (std::map<FiveTuple, FlowId>::const_iterator
       iter = m_flowMap.begin (); iter != m_flowMap.end (); iter++)
    {
	 ...
	 << " sliceNumber=\"" << iter->first.sliceNumber << "\""
         << " />\n";
