...
static const int PfType0AllocationRbg[4] = { 10,       // RGB size 1
		26,       // RGB size 2
		63,       // RGB size 3
		110       // RGB size 4
...
PfSchedulerMemberCschedSapProvider::PfSchedulerMemberCschedSapProvider(
		PfFfMacScheduler* scheduler) :
		m_scheduler(scheduler) {
}

void PfSchedulerMemberCschedSapProvider::CschedCellConfigReq(
		const struct CschedCellConfigReqParameters& params) {
	m_scheduler->DoCschedCellConfigReq(params);
}

void PfSchedulerMemberCschedSapProvider::CschedUeConfigReq(
		const struct CschedUeConfigReqParameters& params) {
	m_scheduler->DoCschedUeConfigReq(params);
}

void PfSchedulerMemberCschedSapProvider::CschedLcConfigReq(
		const struct CschedLcConfigReqParameters& params) {
	m_scheduler->DoCschedLcConfigReq(params);
}

void PfSchedulerMemberCschedSapProvider::CschedLcReleaseReq(
		const struct CschedLcReleaseReqParameters& params) {
	m_scheduler->DoCschedLcReleaseReq(params);
}

void PfSchedulerMemberCschedSapProvider::CschedUeReleaseReq(
		const struct CschedUeReleaseReqParameters& params) {
...
void PfSchedulerMemberSchedSapProvider::SchedDlRlcBufferReq(
		const struct SchedDlRlcBufferReqParameters& params) {
	m_scheduler->DoSchedDlRlcBufferReq(params);
}

void PfSchedulerMemberSchedSapProvider::SchedDlPagingBufferReq(
		const struct SchedDlPagingBufferReqParameters& params) {
	m_scheduler->DoSchedDlPagingBufferReq(params);
}

void PfSchedulerMemberSchedSapProvider::SchedDlMacBufferReq(
		const struct SchedDlMacBufferReqParameters& params) {
	m_scheduler->DoSchedDlMacBufferReq(params);
}

void PfSchedulerMemberSchedSapProvider::SchedDlTriggerReq(
		const struct SchedDlTriggerReqParameters& params) {
	m_scheduler->DoSchedDlTriggerReq(params);
}

void PfSchedulerMemberSchedSapProvider::SchedDlRachInfoReq(
		const struct SchedDlRachInfoReqParameters& params) {
	m_scheduler->DoSchedDlRachInfoReq(params);
}

void PfSchedulerMemberSchedSapProvider::SchedDlCqiInfoReq(
		const struct SchedDlCqiInfoReqParameters& params) {
	m_scheduler->DoSchedDlCqiInfoReq(params);
}

void PfSchedulerMemberSchedSapProvider::SchedUlTriggerReq(
		const struct SchedUlTriggerReqParameters& params) {
	m_scheduler->DoSchedUlTriggerReq(params);
}

void PfSchedulerMemberSchedSapProvider::SchedUlNoiseInterferenceReq(
		const struct SchedUlNoiseInterferenceReqParameters& params) {
	m_scheduler->DoSchedUlNoiseInterferenceReq(params);
}

void PfSchedulerMemberSchedSapProvider::SchedUlSrInfoReq(
		const struct SchedUlSrInfoReqParameters& params) {
	m_scheduler->DoSchedUlSrInfoReq(params);
}

void PfSchedulerMemberSchedSapProvider::SchedUlMacCtrlInfoReq(
		const struct SchedUlMacCtrlInfoReqParameters& params) {
	m_scheduler->DoSchedUlMacCtrlInfoReq(params);
}

void PfSchedulerMemberSchedSapProvider::SchedUlCqiInfoReq(
		const struct SchedUlCqiInfoReqParameters& params) {
...
TypeId PfFfMacScheduler::GetTypeId(void) {
	static TypeId tid =
			TypeId("ns3::PfFfMacScheduler").SetParent<FfMacScheduler>().SetGroupName(
					"Lte").AddConstructor<PfFfMacScheduler>().AddAttribute(
					"CqiTimerThreshold",
					"The number of TTIs a CQI is valid (default 1000 - 1 sec.)",
					UintegerValue(1000),
					MakeUintegerAccessor(
							&PfFfMacScheduler::m_cqiTimersThreshold),
...
void PfFfMacScheduler::SetFfMacCschedSapUser(FfMacCschedSapUser* s) {
	m_cschedSapUser = s;
}

void PfFfMacScheduler::SetFfMacSchedSapUser(FfMacSchedSapUser* s) {
	m_schedSapUser = s;
}
...
void PfFfMacScheduler::SetLteFfrSapProvider(LteFfrSapProvider* s) {
	m_ffrSapProvider = s;
}
...
void PfFfMacScheduler::DoCschedUeConfigReq(
	...
	NS_LOG_FUNCTION(
			this << " RNTI " << params.m_rnti << " txMode " << (uint16_t)params.m_transmissionMode);
...
void PfFfMacScheduler::DoSchedDlRlcBufferReq(
	...
	//Nao eh chamado em cada TTI. Preciso ver isso aqui!
	std::map<LteFlowId_t, FfMacSchedSapProvider::SchedDlRlcBufferReqParameters>::iterator it;
	...
	if (it == m_rlcBufferReq.end()) {
		//Adicionei
		m_slicesOptimizer->SetRowRlcBufferTransmission(params.m_rnti,
				params.m_rlcTransmissionQueueSize);
		m_slicesOptimizer->IncreaseAmountOfSliceData(params.m_rnti,
				params.m_rlcTransmissionQueueSize);
		m_slicesOptimizer->UpdateRlcBufferRetransmission(params.m_rnti,
...
int PfFfMacScheduler::LcActivePerFlow(uint16_t rnti) {
	std::map<LteFlowId_t, FfMacSchedSapProvider::SchedDlRlcBufferReqParameters>::iterator it;
	int lcActive = 0;
...
void PfFfMacScheduler::DoSchedDlTriggerReq(
	...
	NS_LOG_FUNCTION(
			this << " Frame no. " << (params.m_sfnSf >> 4) << " subframe no. " << (0xF & params.m_sfnSf));
	...
	//Adicionei
	map<int, int> RNTI_To_IMSI_Map = m_slicesOptimizer->GetRNTIToIMSIMap();

	RefreshDlCqiMaps();

	m_slicesOptimizer->m_alreadyChosenDynamic = -1;
	...
	for (std::vector<bool>::iterator it = rbgMap.begin(); it != rbgMap.end();
			it++) {
		if ((*it) == true) {
			rbgAllocatedNum++;
		}
	...
	for (std::vector<bool>::iterator it = ulRbMap.begin(); it != ulRbMap.end();
			it++) {
		if ((*it) == true) {
			rbAllocatedNum++;
	...
	for (itRach = m_rachList.begin(); itRach != m_rachList.end(); itRach++) {
		...
		while ((tbSizeBits < (*itRach).m_estimatedSize)
				&& (rbStart + rbLen
						< (ffrRbStartOffset + maxContinuousUlBandwidth))) {
			rbLen++;
		...
		NS_LOG_INFO(
				this << " UL grant allocated to RNTI " << (*itRach).m_rnti << " rbStart " << rbStart << " rbLen " << rbLen << " MCS " << m_ulGrantMcs << " tbSize " << newRar.m_grant.m_tbSize);
	...
	for (uint16_t i = 0; i < m_dlInfoListBuffered.size(); i++) {
		//Insiro aqui - Filtrar o HARQ por Slice
		//#######Adicionei
		/*int rnti = m_dlInfoListBuffered.at(i).m_rnti;
		 int imsi = RNTI_To_IMSI_Map[rnti];
		 if (std::find(ues.begin(), ues.end(), imsi) == ues.end()) {
		 continue; //Esse equipamento não é desse slice
		 }*/
		//#######
		...
		if (retx.at(0) || retx.at(1)) {
			...
			if (free) {
				...
				if (j == dciRbg.size()) {
					...
					rbgMap = rbgMapCopy;
					NS_LOG_INFO(
							this << " Move retx in RBGs " << dciRbg.size ());
				} else {
...
*						itHarq).second.at (harqId).m_rv.at (j)++;
			...
			NS_LOG_INFO(
					this << " HARQ received ACK for UE " << m_dlInfoListBuffered.at (i).m_rnti);
			std::map<uint16_t, DlHarqProcessesStatus_t>::iterator it =
	...
	//###################################
	//printf("%d\t%d\t%lu\n", m_slicesOptimizer->m_CurrentFrame,  m_slicesOptimizer->m_CurrentSubFrame, m_slicesOptimizer->m_SliceID_to_SliceData.size());
	int amountAux = m_slicesOptimizer->GetAmountOfBufferData(82);
	if(m_slicesOptimizer->m_CurrentFrame == 321 and m_slicesOptimizer->m_CurrentSubFrame == 4){
		int k=5;
		k++;
	}
	amountAux++;
	m_slicesOptimizer->ComputeSliceDataSize();
	int numberOfUsedRBGs = rbgAllocatedNum;
	int numberOfUsedRBs = rbgAllocatedNum;
	int auxMCS=0;

	/*uint16_t i;
	int sum;
	bool flagErase = false;
	if(m_slicesOptimizer->m_SliceID_to_SliceData[2] > 0){
		flagErase = true;
		printf("Antes:%d %d\n", m_slicesOptimizer->m_CurrentFrame, m_slicesOptimizer->m_CurrentSubFrame);
		for(i=1; i<= 5; i++){
			sum = m_slicesOptimizer->m_SliceID_to_SliceData[i];
			printf("slice: %d %d\n", i, sum);
		}
			int k = 5;
			k++;
	...
	for (int i = 0; i < rbgNum; i++) {
		std::vector<int> ues = m_slicesOptimizer->SelectSlice(i); //Verificar para cada RBG
		...
		if (rbgMap.at(i) == false) {
			...
			for (it = m_flowStatsDl.begin(); it != m_flowStatsDl.end(); it++) {
				...
				if (true) {
					if (m_slicesOptimizer->GetAmountOfBufferData(it->first)
							<= 0) {
						continue;
					}
				}
				...
				if (m_slicesOptimizer->GetAmountOfSliceData((*it).first) <= 0) {
					break;
				}
				//#######
				...
				if ((itRnti != rntiAllocated.end())
					...
					if (itRnti != rntiAllocated.end()) {
						NS_LOG_DEBUG(
								this << " RNTI discared for HARQ tx" << (uint16_t)(*it).first);
					}
					if (!HarqProcessAvailability((*it).first)) {
						NS_LOG_DEBUG(
								this << " RNTI discared for HARQ id" << (uint16_t)(*it).first);
					}
					continue;
				}
				...
				if ((cqi1 > 0) || (cqi2 > 0)) // CQI == 0 means "out of range" (see table 7.2.3-1 of 36.213)
					...
					if (LcActivePerFlow((*it).first) > 0) {
						...
						NS_LOG_INFO(
								this << " RNTI " << (*it).first << " MCS " << (uint32_t)mcs << " achievableRate " << achievableRate << " avgThr " << (*it).second.lastAveragedThroughput << " RCQI " << rcqi);

						if (rcqi > rcqiMax) { //Pega o fluxo com maior métrica (rcqi ou achievableRate)
							auxMCS = mcs;
							tbs = m_amc->GetTbSizeFromMcs(mcs, rbgSize) / 8; //Adicionei
							rcqiMax = rcqi;
							itMax = it;
							if(alreadyCountedRBG == false){
								numberOfUsedRBGs++;
								alreadyCountedRBG = true;
							}
						}
					}
				}   // end if cqi
			...
			if (itMax == m_flowStatsDl.end()) {
				...
				//Me parece que aqui já aloca.
				int numberRealUsedPRBs =
				m_amc->GetPRBEstimationFromMCSAndTBS(auxMCS, m_slicesOptimizer->GetAmountOfBufferData((*itMax).first) * 8, rbgSize);
				numberOfUsedRBs += numberRealUsedPRBs;
				m_slicesOptimizer->DecreaseAmountOfSliceData((*itMax).first,
						m_slicesOptimizer->UpdateRowRlcBuffer((*itMax).first, tbs,
								false));
				rbgMap.at(i) = true;
	...
	if(numberOfUsedRBGs > 0){
		std::string downlinkTrace = m_slicesOptimizer->GetUsedRBsDownlinkTrace();
		if(downlinkTrace.size() > 1 ){
			std::ofstream outfile (downlinkTrace, ios::out|ios::app);
			outfile << m_slicesOptimizer->m_CurrentFrame << "\t";
			outfile << m_slicesOptimizer->m_CurrentSubFrame << "\t";
			outfile << numberOfUsedRBs<< std::endl;
			outfile.close();
		}
		//printf("%d\t%d\t%d\n", m_uplinkSlicesOptimizer->m_CurrentFrame, m_uplinkSlicesOptimizer->m_CurrentSubFrame, nbAllocatedRBs);
	}
	...
	while (itMap != allocationMap.end()) {
		...
		NS_LOG_INFO(
				this << "Allocate user " << newEl.m_rnti << " rbg " << lcActives);
		if (lcActives == 0) {
		...
		if (itCqi != m_a30CqiRxed.end()) {
			for (uint16_t k = 0; k < (*itMap).second.size(); k++) {
				if ((*itCqi).second.m_higherLayerSelected.size()
						> (*itMap).second.at(k)) {
					NS_LOG_INFO(
							this << " RBG " << (*itMap).second.at (k) << " CQI " << (uint16_t)((*itCqi).second.m_higherLayerSelected.at ((*itMap).second.at (k)).m_sbCqi.at (0)));
		...
		for (uint8_t j = 0; j < nLayer; j++) {
			NS_LOG_INFO(
					this << " Layer " << (uint16_t)j << " CQI selected " << (uint16_t)worstCqi.at (j));
		}
		...
		for (uint8_t j = 0; j < nLayer; j++) {
			...
			NS_LOG_INFO(
					this << " Layer " << (uint16_t)j << " MCS selected" << m_amc->GetMcsFromCqi (worstCqi.at (j)));
			bytesTxed += tbSize;
		}
		...
		for (itBufReq = m_rlcBufferReq.begin();
			...
			if (((*itBufReq).first.m_rnti == (*itMap).first)
				...
				for (uint8_t j = 0; j < nLayer; j++) {
					...
					NS_LOG_INFO(
							this << " LCID " << (uint32_t) newRlcEl.m_logicalChannelIdentity << " size " << newRlcEl.m_size << " layer " << (uint16_t)j);
		...
		if (it != m_flowStatsDl.end()) {
			...
			NS_LOG_INFO(
					this << " UE total bytes txed " << (*it).second.lastTtiBytesTrasmitted);
				...
				* (*itStats).second.lastAveragedThroughput)
				+ ((1.0 / m_timeWindow)
						* (double) ((*itStats).second.lastTtiBytesTrasmitted
								/ 0.001));
		NS_LOG_INFO(
				this << " UE total bytes " << (*itStats).second.totalBytesTransmitted);
		NS_LOG_INFO(
				this << " UE average throughput " << (*itStats).second.lastAveragedThroughput);
		(*itStats).second.lastTtiBytesTrasmitted = 0;
	}
	//m_slicesOptimizer->ClearSlicesData();
	m_schedSapUser->SchedDlConfigInd(ret);
	m_slicesOptimizer->m_alreadyChosenDynamic = -1; //So para o Dinamico sem RBG
	m_slicesOptimizer->m_lastSliceFromArray = -1;
	m_slicesOptimizer->m_lastSliceSelected = -1;
...
void PfFfMacScheduler::DoSchedDlCqiInfoReq(
	...
	for (unsigned int i = 0; i < params.m_cqiList.size(); i++) {
		if (params.m_cqiList.at(i).m_cqiType == CqiListElement_s::P10) {
			NS_LOG_LOGIC(
					"wideband CQI " << (uint32_t) params.m_cqiList.at (i).m_wbCqi.at (0) << " reported");
...
std::map<uint16_t, bool> PfFfMacScheduler::ComputeCQI(int baseRB, int offset){

	  	  	std::map<uint16_t, bool> m_imsiToStatus;
			map<int, int> RNTI_To_IMSI_Map = m_uplinkSlicesOptimizer->GetRNTIToIMSIMap();
			std::map<uint16_t, std::vector<double> >::iterator itCqi;
			int cqi = 0;
			for (itCqi = m_ueCqi.begin(); itCqi != m_ueCqi.end(); itCqi++) {

					double minSinr = (*itCqi).second.at(baseRB);
					if (minSinr == NO_SINR) {
						minSinr = EstimateUlSinr((*itCqi).first, baseRB);
					}
					for (uint16_t i = baseRB;
							i < baseRB + offset; i++) {
						double sinr = (*itCqi).second.at(i);
						if (sinr == NO_SINR) {
							sinr = EstimateUlSinr((*itCqi).first, i);
						}
					...
					if (cqi == 0) {
						m_imsiToStatus[RNTI_To_IMSI_Map[(*itCqi).first]] = false;
					}
					else{
						m_imsiToStatus[RNTI_To_IMSI_Map[(*itCqi).first]] = true;
					}
			}

			return m_imsiToStatus;
}
...
void PfFfMacScheduler::DoSchedUlTriggerReq(
	...
	NS_LOG_FUNCTION(
			this << " UL - Frame no. " << (params.m_sfnSf >> 4) << " subframe no. " << (0xF & params.m_sfnSf) << " size " << params.m_ulInfoList.size ());
	...
	m_rachAllocationMap.resize(m_cschedCellConfig.m_ulBandwidth, 0);
	map<int, int> RNTI_To_IMSI_Map = m_uplinkSlicesOptimizer->GetRNTIToIMSIMap();
	map<int, int> IMSI_To_RNTI_Map = m_uplinkSlicesOptimizer->GetIMSIToRNTIMap();

	rbMap.resize(m_cschedCellConfig.m_ulBandwidth, false);
	rbMap = m_ffrSapProvider->GetAvailableUlRbg();

	for (std::vector<bool>::iterator it = rbMap.begin(); it != rbMap.end();
			it++) {
		if ((*it) == true) {
			rbAllocatedNum++;
		}
	...
	uint8_t minContinuousUlBandwidth =
		...
		for (uint16_t i = 0; i < params.m_ulInfoList.size(); i++) {
			if (params.m_ulInfoList.at(i).m_receptionStatus
				...
				uint8_t harqId = (uint8_t) ((*itProcId).second - HARQ_PERIOD)
						% HARQ_PROC_NUM;
				NS_LOG_INFO(
						this << " UL-HARQ retx RNTI " << rnti << " harqId " << (uint16_t)harqId << " i " << i << " size " << params.m_ulInfoList.size ());
				...
				if (free) {
					...
					NS_LOG_INFO(
							this << " Send retx in the same RBs " << (uint16_t)dci.m_rbStart << " to " << dci.m_rbStart + dci.m_rbLen << " RV " << (*itStat).second.at (harqId) + 1);
				} else {
					NS_LOG_INFO(
				...
				NS_LOG_INFO(
						this << " HARQ-ACK feedback from RNTI " << params.m_ulInfoList.at (i).m_rnti);
			}
		}
	}

	if(m_uplinkSlicesOptimizer->m_CurrentSubFrame == 1 && m_uplinkSlicesOptimizer->m_CurrentFrame == 10){
		int k =0;
		k++;

	}
	//printf("%d \t %d \n", m_uplinkSlicesOptimizer->m_CurrentFrame, m_uplinkSlicesOptimizer->m_CurrentSubFrame);
	std::map<uint16_t, uint32_t>::iterator it;
	int nflows = 0;
	for (it = m_ceBsrRxed.begin(); it != m_ceBsrRxed.end(); it++) {
		m_uplinkSlicesOptimizer->SetRowRlcBufferTransmission(it->first, it->second); //Depois modificar o nome!
		m_uplinkSlicesOptimizer->IncreaseAmountOfSliceData(it->first, it->second);
	}

	m_uplinkSlicesOptimizer->m_alreadyChosenDynamicUplink = -1;
	int sum = 0;
	//bool flag = false; //Se true, o que foi servido da última vez pode ser servido denovo.
	//m_uplinkSlicesOptimizer->PopulateCQIStatus(ComputeCQI(0, 100));//Adicionei
	//ZZZZZ
	//if(m_ceBsrRxed.count(177) > 0){
		//printf("%d\n", m_ceBsrRxed[177]);
	//}
	for (it = m_ceBsrRxed.begin(); it != m_ceBsrRxed.end(); it++) {
		//#######Adicionei --- Comentei
		std::vector<int> ues = m_uplinkSlicesOptimizer->SelectUplinkSlice(0);
		int rnti = (*it).first;
		int imsi = RNTI_To_IMSI_Map[rnti];
		if (std::find(ues.begin(), ues.end(), imsi) == ues.end()) {
			continue; //Esse equipamento não é desse slice
		...
		if (((*it).second > 0) && (itRnti == rntiAllocated.end())) {
			/*if(RNTI_To_IMSI_Map[(*it).first] > 21 && RNTI_To_IMSI_Map[(*it).first] <= 50){//Remover depois
			 printf("%d", (*it).first);
			 }*/
			sum += (*it).second;
			nflows++;
	...
	//ZZZZZ
	if (nflows == 0
			&& m_uplinkSlicesOptimizer->GetRBGFlag() == true &&
			m_uplinkSlicesOptimizer->GetDynamicSlicingFlag() == false) { //Se o ZZZZZ acima não contabilizar um fluxo no Estático + RBG
		std::set<int> slices =
				m_uplinkSlicesOptimizer->GetSlicesPresentInSubframe();
		std::set<int>::iterator it_set;
		for (it_set = slices.begin(); it_set != slices.end(); it_set++) {
			std::vector<int> ues = m_uplinkSlicesOptimizer->GetSlicebyID(
					*it_set);
			int rnti;
			int imsi;
			for (it = m_ceBsrRxed.begin(); it != m_ceBsrRxed.end(); it++) {
				rnti = (*it).first;
				imsi = RNTI_To_IMSI_Map[rnti];
				if (std::find(ues.begin(), ues.end(), imsi) == ues.end()) {
					continue; //Esse equipamento não é desse slice
				}
				//#######
	...
	//Time instant = Simulator::Now();
	//std::cout << "INICIO:!!" << instant.GetNanoSeconds() << "\n";
	...
	uint16_t rbPerFlow =
			(minContinuousUlBandwidth < tempRbPerFlow) ?
					minContinuousUlBandwidth : tempRbPerFlow;
	...
	//srand(time(0));
	std::vector<int> ues = m_uplinkSlicesOptimizer->SelectUplinkSlice(rbAllocated);
	std::random_shuffle(ues.begin(), ues.end()); //OK!
	m_nextRntiUl = IMSI_To_RNTI_Map[ues[0]];

	for (it = m_ceBsrRxed.begin(); it != m_ceBsrRxed.end(); it++) {
		if ((*it).first == m_nextRntiUl) //Pega o que está no m_nextRntiUl.
				{
			break;
		}
	}

	int lastRNTI = 0; //Adicionei
	int nextRBToBeAllocated = rbAllocatedNum; //Adicionei
	size_t i_aux = 0; //Adicionei
	std::set<int> ueAlreadyAllocated;
	//int lastAllocated; //Se for o anterior, pode alocar denovo. Me parece que isso aqui piora.
	uint16_t  rbPerFlowAux = rbPerFlow;
	int nbAllocatedRBs = 0;

	//Adicionei o laco FOR abaixo:
	//printf("Tem trafego: ");
	/*for (it = m_ceBsrRxed.begin(); it != m_ceBsrRxed.end(); it++) { //Cuidado com o iterator, pois vai interferir na frente!
		if((*it).second > 0){
			printf("%d \t", RNTI_To_IMSI_Map[(*it).first] );
			sum += (*it).second;
		}
	}*/
	//printf("\nANTES: ");
	//printf("%d \t %d \t %d\n", m_uplinkSlicesOptimizer->m_CurrentFrame, m_uplinkSlicesOptimizer->m_CurrentSubFrame, sum);
	bool alreadySetFlag = false;
	int offset;
	int lastAllocated = -1;
	//bool flag = false;
	std::map <int, UlDciListElement_s> rntiToDciList; //Apenas no RBG

	//####################################################
	//Talvez retirar
	/*std::map<uint16_t, uint32_t>::iterator it_2;
	int nflows_aux = 0;
	for (it_2 = m_ceBsrRxed.begin(); it_2 != m_ceBsrRxed.end(); it_2++) {
		std::set<uint16_t>::iterator itRnti = rntiAllocated.find(
				(*it_2).first);
		if (((*it_2).second > 0) && (itRnti == rntiAllocated.end())) {
			nflows_aux++;
			//break;
		}
	}*/
	...
	do {
		if(m_uplinkSlicesOptimizer->GetRBGFlag() == true && alreadySetFlag == false){
			offset = m_uplinkSlicesOptimizer->CalculateOffset(nextRBToBeAllocated);

			/*if(nflows_aux == 1){
				rbPerFlow = m_cschedCellConfig.m_ulBandwidth - nextRBToBeAllocated;
				std::cout << "CHEGOU!\n";
			}*/

			if(rbPerFlowAux < offset){
				rbPerFlow = rbPerFlowAux;
			}
			else{
				rbPerFlow = offset;
			}
			//m_uplinkSlicesOptimizer->PopulateCQIStatus(ComputeCQI(nextRBToBeAllocated, rbPerFlow));
			alreadySetFlag = true;
		}
		//flag = false;
		std::vector<int> ues_aux = m_uplinkSlicesOptimizer->SelectUplinkSlice(nextRBToBeAllocated);
		//Fazer offset = rbPerFlow
		std::set<int> ues(ues_aux.begin(), ues_aux.end());
		//#Adicionei
		//#Adicionei por ultimo
		int rnti = (*it).first;
		int imsi = RNTI_To_IMSI_Map[rnti];
		if (ues.find(imsi) == ues.end() || (ueAlreadyAllocated.count(rnti) == 1 && lastAllocated != rnti)) {
			//std::advance(it, rand() % m_ceBsrRxed.size());
			it++;
			...
			i_aux++;
			if(i_aux >= m_uplinkSlicesOptimizer->GetNumberOfUEs()){ //Aqui
				if(m_uplinkSlicesOptimizer->GetRBGFlag() == true){
					//Adicionei Hoje
					int i;
					for(i = nextRBToBeAllocated; i < (nextRBToBeAllocated + offset); i++){
						rbMap.at(i) = true;
					}
					//####################
					i_aux = 0;
					nextRBToBeAllocated += offset;
					alreadySetFlag = false;
					lastAllocated = -1;//24_01

					if(nextRBToBeAllocated >= m_cschedCellConfig.m_ulBandwidth){
						break;
					}
				}
		...
		//Adicionei hoje
		/*if(ueAlreadyAllocated.count(rnti) == 1){
			it++;
			if (it == m_ceBsrRxed.end()) {
				// restart from the first
				it = m_ceBsrRxed.begin();
		...
		if ((itRnti != rntiAllocated.end()) || ((*it).second == 0)) {
			if (lastRNTI == rnti) { //Adicionei. Esse já foi servido e agora não tem mais nada para transmitir;
				break;
			}
			// UE already allocated for UL-HARQ -> skip it
			NS_LOG_DEBUG(
					this << " UE already allocated in HARQ -> discared, RNTI " << (*it).first);
			it++;
			if (it == m_ceBsrRxed.end()) {
				// restart from the first
				it = m_ceBsrRxed.begin();
		...
		while ((!allocated)
				&& ((rbAllocated + rbPerFlow - m_cschedCellConfig.m_ulBandwidth)
						< 1) && (rbPerFlow != 0)) {
			// check availability
			bool free = true;
			...
			if (free) {
				NS_LOG_INFO(
						this << "RNTI: "<< (*it).first<< " RB Allocated " << rbAllocated << " rbPerFlow " << rbPerFlow << " flows " << nflows);
				...
				ueAlreadyAllocated.insert(it->first);
				allocated = true;
				break;
			}
			rbAllocated++;
		...
		//Daqui para frente nao mexe mais com a alocacao do RB
		if (!allocated) {
...
//          return;
			...
			double s = log2(
					1
							+ (std::pow(10, minSinr / 10)
									/ ((-std::log(5.0 * 0.00005)) / 1.5)));
			cqi = m_amc->GetCqiFromSpectralEfficiency(s);
			/*if(cqi == 0){ //Depois remover
				cqi = 1;
			}*/
			//printf("CQI: %d \t %d\n", RNTI_To_IMSI_Map[(*it).first], cqi);
			if (cqi == 0) {
				it++;
				if (it == m_ceBsrRxed.end()) {
					// restart from the first
				...
				NS_LOG_DEBUG(
						this << " UE discared for CQI=0, RNTI " << uldci.m_rnti);
		...
		alreadySetFlag = false;
		nextRBToBeAllocated = rbAllocated;
		uldci.m_tbSize = (m_amc->GetTbSizeFromMcs(uldci.m_mcs, rbPerFlow) / 8);
		int numberUsedRealUsedPRBs =
				m_amc->GetPRBEstimationFromMCSAndTBS(uldci.m_mcs, m_uplinkSlicesOptimizer->GetAmountOfBufferData(uldci.m_rnti) * 8, rbPerFlow);
		nbAllocatedRBs+= numberUsedRealUsedPRBs;
		//printf("%d\t%d\t%d\t%d\n", m_uplinkSlicesOptimizer->m_CurrentFrame, m_uplinkSlicesOptimizer->m_CurrentSubFrame,
		//		m_uplinkSlicesOptimizer->GetAmountOfBufferData(uldci.m_rnti), uldci.m_tbSize);
		int bufferSizeDecrease = m_uplinkSlicesOptimizer->GetAmountOfBufferData(uldci.m_rnti);
		if(uldci.m_tbSize < bufferSizeDecrease){
			bufferSizeDecrease = uldci.m_tbSize;
		}

		if(m_uplinkSlicesOptimizer->GetRBGFlag() == true){//Adicionei 23/01
			UlDciListElement_s dciAux;
			if(rntiToDciList.count(uldci.m_rnti) == 0){
				dciAux.m_rbStart = uldci.m_rbStart;
				dciAux.m_mcs = uldci.m_mcs; //Se muito alto, faz perder pacotes. Em algumas linhas a coluna ndi fica 0 no trace (UlTxPhyStats.txt)
				dciAux.m_rnti =  uldci.m_rnti;
				dciAux.m_rbLen = rbPerFlow;
				dciAux.m_tbSize = uldci.m_tbSize;
				dciAux.m_ndi = 1;
				dciAux.m_cceIndex = 0;
				dciAux.m_aggrLevel = 1;
				dciAux.m_ueTxAntennaSelection = 3; // antenna selection OFF
				dciAux.m_hopping = false;
				dciAux.m_n2Dmrs = 0;
				dciAux.m_tpc = 0; // no power control
				dciAux.m_cqiRequest = false; // only period CQI at this stage
				dciAux.m_ulIndex = 0; // TDD parameter
				dciAux.m_dai = 1; // TDD parameter
				dciAux.m_freqHopping = 0;
				dciAux.m_pdcchPowerOffset = 0; // not used
				rntiToDciList[uldci.m_rnti] = dciAux;
				if(dciAux.m_rbStart + dciAux.m_rbLen > 100){
					int offset = (dciAux.m_rbStart + dciAux.m_rbLen) - 100;
					dciAux.m_rbLen -= offset;
					printf("%d \t %d \n", m_uplinkSlicesOptimizer->m_CurrentFrame, m_uplinkSlicesOptimizer->m_CurrentSubFrame);
					std::cout << dciAux.m_rbStart + dciAux.m_rbLen << "\n";
				}
			}
			else{
				dciAux = rntiToDciList[uldci.m_rnti];
				if(dciAux.m_mcs > uldci.m_mcs){
					dciAux.m_mcs = uldci.m_mcs; //Aqui vai ter atualizar os tbs anteriores desse usuario, pois o MCS foi diminuido.
		...
		m_uplinkSlicesOptimizer->DecreaseAmountOfSliceData(uldci.m_rnti, bufferSizeDecrease);
		m_uplinkSlicesOptimizer->UpdateRowRlcBuffer(it->first, bufferSizeDecrease, false);
		UpdateUlRlcBufferInfo(uldci.m_rnti, uldci.m_tbSize);

		//if(m_ceBsrRxed[uldci.m_rnti] == 0){
			//nflows_aux --;
		//}*/
		...
		uldci.m_freqHopping = 0;
		uldci.m_pdcchPowerOffset = 0; // not used
		if(m_uplinkSlicesOptimizer->GetRBGFlag() == false){
		...
		if (m_harqOn == true) {
			...
			if (itStat == m_ulHarqProcessesStatus.end()) {
				NS_LOG_ERROR(
						"No info find in HARQ buffer for UE (might change eNB) " << uldci.m_rnti);
			}
			(*itStat).second.at(harqId) = 0;
		}

		NS_LOG_INFO(
				this << " UE Allocation RNTI " << (*it).first << " startPRB " << (uint32_t)uldci.m_rbStart << " nPRB " << (uint32_t)uldci.m_rbLen << " CQI " << cqi << " MCS " << (uint32_t)uldci.m_mcs << " TBsize " << uldci.m_tbSize << " RbAlloc " << rbAllocated << " harqId " << (uint16_t)harqId);
	...
	} while (((*it).first != m_nextRntiUl) || /*((m_cschedCellConfig.m_ulBandwidth - rbAllocated) != 0)*/ rbAllocated < m_cschedCellConfig.m_ulBandwidth);
	//printf("%d \t %d \t %d \n", m_uplinkSlicesOptimizer->m_CurrentFrame, m_uplinkSlicesOptimizer->m_CurrentSubFrame, rbAllocated);


	//Adicionei o FOR abaixo:
	sum = 0;
	for (it = m_ceBsrRxed.begin(); it != m_ceBsrRxed.end(); it++) {
		sum += (*it).second;
	}
	//printf("DEPOIS: ");
	//printf("%d \t %d \t %d \t %d\n", m_uplinkSlicesOptimizer->m_CurrentFrame, m_uplinkSlicesOptimizer->m_CurrentSubFrame, rbAllocated, sum);
	//printf("%d \t %d \t %d \n", m_uplinkSlicesOptimizer->m_CurrentFrame, m_uplinkSlicesOptimizer->m_CurrentSubFrame, sum);
	std::string uplinkTrace = m_uplinkSlicesOptimizer->GetUsedRBsUplinkTrace();
	if (uplinkTrace.size() > 1) {
		std::ofstream outfile(uplinkTrace, ios::out | ios::app);
		outfile << m_uplinkSlicesOptimizer->m_CurrentFrame << "\t";
		outfile << m_uplinkSlicesOptimizer->m_CurrentSubFrame << "\t";
		outfile << nbAllocatedRBs << "\t";
		outfile << sum << std::endl; //Tenho que transmitir
		outfile.close();
		//printf("%d\t%d\t%d\n", m_uplinkSlicesOptimizer->m_CurrentFrame, m_uplinkSlicesOptimizer->m_CurrentSubFrame, nbAllocatedRBs);
	}
	...
	if(m_uplinkSlicesOptimizer->GetRBGFlag() == true){
		...
		for (std::map<int, UlDciListElement_s>::iterator itDCI =
				rntiToDciList.begin(); itDCI != rntiToDciList.end(); itDCI++) {
			ret.m_dciList.push_back(itDCI->second);
		}
	}

	//cout << ret.m_dciList.size() << "\n";
	//if(ret.m_dciList.size() > 0){
	//	if(ret.m_dciList[0].m_rnti == 3 && ret.m_dciList[0].m_mcs == 10){
			//cout << "aaa";
	//	}
	//}
	...
	m_allocationMaps.insert(
			std::pair<uint16_t, std::vector<uint16_t> >(params.m_sfnSf,
					rbgAllocationMap));
	m_schedSapUser->SchedUlConfigInd(ret);
	m_uplinkSlicesOptimizer->ClearSlicesData();
	m_uplinkSlicesOptimizer->m_alreadyChosenDynamicUplink = -1;
	return;
}
...
// retrieve the allocation for this subframe
	...
	case UlCqi_s::PUSCH: {
		...
		std::map<uint16_t, std::vector<double> >::iterator itCqi;
		NS_LOG_DEBUG(
				this << " Collect PUSCH CQIs of Frame no. " << (params.m_sfnSf >> 4) << " subframe no. " << (0xF & params.m_sfnSf));
		...
		for (uint32_t i = 0; i < (*itMap).second.size(); i++) {
			...
			if (itCqi == m_ueCqi.end()) {
				...
				NS_LOG_DEBUG(
						this << " RNTI " << (*itMap).second.at (i) << " RB " << i << " SINR " << sinr);
				// update correspondent timer
				std::map<uint16_t, uint32_t>::iterator itTimers;
	...
	case UlCqi_s::SRS: {
		NS_LOG_DEBUG(
				this << " Collect SRS CQIs of Frame no. " << (params.m_sfnSf >> 4) << " subframe no. " << (0xF & params.m_sfnSf));
		...
		if (itCqi == m_ueCqi.end()) {
			...
			for (uint32_t j = 0; j < m_cschedCellConfig.m_ulBandwidth; j++) {
				...
				newCqi.push_back(sinr);
				NS_LOG_INFO(
						this << " RNTI " << rnti << " new SRS-CQI for RB  " << j << " value " << sinr);
			...
			for (uint32_t j = 0; j < m_cschedCellConfig.m_ulBandwidth; j++) {
				...
				NS_LOG_INFO(
						this << " RNTI " << rnti << " update SRS-CQI for RB  " << j << " value " << sinr);
			}
			// update correspondent timer
			std::map<uint16_t, uint32_t>::iterator itTimers;
...
void PfFfMacScheduler::RefreshDlCqiMaps(void) {
	...
	while (itP10 != m_p10CqiTimers.end()) {
		NS_LOG_INFO(
				this << " P10-CQI for user " << (*itP10).first << " is " << (uint32_t)(*itP10).second << " thr " << (uint32_t)m_cqiTimersThreshold);
		if ((*itP10).second == 0) {
	...
	while (itA30 != m_a30CqiTimers.end()) {
		NS_LOG_INFO(
				this << " A30-CQI for user " << (*itA30).first << " is " << (uint32_t)(*itA30).second << " thr " << (uint32_t)m_cqiTimersThreshold);
		if ((*itA30).second == 0) {
			// delete correspondent entries
...
void PfFfMacScheduler::RefreshUlCqiMaps(void) {
	...
	while (itUl != m_ueCqiTimers.end()) {
		NS_LOG_INFO(
				this << " UL-CQI for user " << (*itUl).first << " is " << (uint32_t)(*itUl).second << " thr " << (uint32_t)m_cqiTimersThreshold);
		if ((*itUl).second == 0) {
			// delete correspondent entries
...
void PfFfMacScheduler::UpdateDlRlcBufferInfo(uint16_t rnti, uint8_t lcid,
		uint16_t size) {
	std::map<LteFlowId_t, FfMacSchedSapProvider::SchedDlRlcBufferReqParameters>::iterator it;
	...
	if (it != m_rlcBufferReq.end()) {
		NS_LOG_INFO(
				this << " UE " << rnti << " LC " << (uint16_t)lcid << " txqueue " << (*it).second.m_rlcTransmissionQueueSize << " retxqueue " << (*it).second.m_rlcRetransmissionQueueSize << " status " << (*it).second.m_rlcStatusPduSize << " decrease " << size);
		...
		if (((*it).second.m_rlcStatusPduSize > 0)
			...
			m_slicesOptimizer->UpdateRlcBufferRetransmission(rnti, 0);
			(*it).second.m_rlcRetransmissionQueueSize = 0;
			...
			if ((*it).second.m_rlcTransmissionQueueSize <= size - rlcOverhead) {
				//Aqui
				m_slicesOptimizer->SetRowRlcBufferTransmission(rnti, 0);
				(*it).second.m_rlcTransmissionQueueSize = 0;
			} else {
				//Aqui
				//m_slicesOptimizer->UpdateRowRlcBuffer(rnti, size - rlcOverhead,
				//		false);
		...
		NS_LOG_ERROR(
				this << " Does not find DL RLC Buffer Report of UE " << rnti);
	}
}
...
void PfFfMacScheduler::UpdateUlRlcBufferInfo(uint16_t rnti, uint16_t size) {
	...
	if (it != m_ceBsrRxed.end()) {
		NS_LOG_INFO(
				this << " UE " << rnti << " size " << size << " BSR " << (*it).second);
		if ((*it).second >= size) {
			(*it).second -= size;
		} else {
			(*it).second = 0;
...
void PfFfMacScheduler::TransmissionModeConfigurationUpdate(uint16_t rnti,
		uint8_t txMode) {
