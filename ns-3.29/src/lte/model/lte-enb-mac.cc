...
LteEnbMac::LteEnbMac ()
{
  ...
  m_uplinkSlicesOptimizer = new SlicesOptimizer();
  m_slicesOptimizer = new SlicesOptimizer();
...
SlicesOptimizer*
LteEnbMac::GetSlicesOptimizer(){
	return m_slicesOptimizer;
}

SlicesOptimizer*
LteEnbMac::GetUplinkSlicesOptimizer(){
	return m_uplinkSlicesOptimizer;
}
...
LteEnbMac::DoSubframeIndication (uint32_t frameNo, uint32_t subframeNo)
{
  ...
  //###Adicionei###
  m_slicesOptimizer->SetCurrentFrame(m_frameNo);
  m_slicesOptimizer->SetCurrentSubFrame(m_subframeNo);
  m_uplinkSlicesOptimizer->SetCurrentFrame(m_frameNo);
  m_uplinkSlicesOptimizer->SetCurrentSubFrame(m_subframeNo);
  //###############
  m_schedSapProvider->SchedDlTriggerReq (dlparams); //Começa o escalonador
  //m_slicesOptimizer->ClearRowRlcBuffer();
  // --- UPLINK ---
