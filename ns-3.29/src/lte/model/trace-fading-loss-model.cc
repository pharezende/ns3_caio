...
void //Adicionei
TraceFadingLossModel::LoadTrace(std::string fileName, int traceID) {
	NS_LOG_FUNCTION(this << "Loading Fading Trace " << m_traceFile);
	std::ifstream ifTraceFile;
	ifTraceFile.open(fileName.c_str(), std::ifstream::in);
	FadingTrace fadingTraceAux;
	if (!ifTraceFile.good()) {
		NS_LOG_INFO(this << " File: " << m_traceFile);
		NS_ASSERT_MSG(ifTraceFile.good(), " Fading trace file not found");
	}
...
//   NS_LOG_INFO (this << " RB " << (uint32_t)m_rbNum << " samples " << m_samplesNum);
	for (uint32_t i = 0; i < m_rbNum; i++) {
		...
		fadingTraceAux.push_back(rbTimeFadingTrace);
	}
	m_fadingTraceMap[traceID] = fadingTraceAux;
	//m_timeGranularity = m_traceLength.GetMilliSeconds () / m_samplesNum;
	//m_lastWindowUpdate = Simulator::Now ();
}
  ...
  while (vit != rxPsd->ValuesEnd ())
    {
      ...
      if (*vit != 0.)
        {
    	  //
          double fading = m_fadingTraceMap.at(traceID).at (subChannel).at (index);
          //std::cout << subChannel << "\t" << index << "\t" << fading << "\n";
