    ...
    case INITIAL_RANDOM_ACCESS:
      {
        ...
        if (m_rrc->m_admitRrcConnectionRequest == true)
          ...
          {
            NS_LOG_INFO ("rejectingSlicesOptimizer* m_sliceOptimizer; //Adicionei connection request for RNTI " << m_rnti);
...
UeManager::RecvRrcConnectionSetupCompleted (LteRrcSap::RrcConnectionSetupCompleted msg)
{
  ...
  switch (m_state)
    ...
    case CONNECTION_SETUP:
      ...
      m_rrc->m_sliceOptimizer->InsertRowRNTIToIMSIMap(m_rnti, m_imsi);
      m_rrc->m_sliceOptimizer->InsertRowIMSIToRNTIMap(m_imsi, m_rnti);
      m_rrc->m_uplinkSliceOptimizer->InsertRowRNTIToIMSIMap(m_rnti, m_imsi);
      m_rrc->m_uplinkSliceOptimizer->InsertRowIMSIToRNTIMap(m_imsi, m_rnti);
      break;
...
void
LteEnbRrc::SetSliceOptimizer(SlicesOptimizer* sliceOptimizer){ //Adicionei
	m_sliceOptimizer = sliceOptimizer;
}

void
LteEnbRrc::SetUplinkSliceOptimizer(SlicesOptimizer* sliceOptimizer){ //Adicionei
	m_uplinkSliceOptimizer = sliceOptimizer;
}
