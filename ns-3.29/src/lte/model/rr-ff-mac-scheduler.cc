...
void RrFfMacScheduler::DoCschedUeReleaseReq(
	...
	while (it != m_rlcBufferReq.end()) {
		if ((*it).m_rnti == params.m_rnti) {
			NS_LOG_INFO(
					this << " Erase RNTI " << (*it).m_rnti << " LC " << (uint16_t)(*it).m_logicalChannelIdentity);
			it = m_rlcBufferReq.erase(it);
		} else {
			it++;
		}
...
void RrFfMacScheduler::DoSchedDlRlcBufferReq(
	...
	NS_LOG_INFO(
			this << " RNTI " << params.m_rnti << " LC " << (uint16_t)params.m_logicalChannelIdentity << " RLC tx size " << params.m_rlcTransmissionQueueHolDelay << " RLC retx size " << params.m_rlcRetransmissionQueueSize << " RLC stat size " << params.m_rlcStatusPduSize);
...
//Aqui é aonde aloca os recursos sem fio aos UEs
void RrFfMacScheduler::DoSchedDlTriggerReq(
		const struct FfMacSchedSapProvider::SchedDlTriggerReqParameters& params) {
	NS_LOG_FUNCTION(
			this << " DL Frame no. " << (params.m_sfnSf >> 4) << " subframe no. " << (0xF & params.m_sfnSf));
	// API generated by RLC for triggering the scheduling of a DL subframe
	RefreshDlCqiMaps();
	int rbgSize = GetRbgSize(m_cschedCellConfig.m_dlBandwidth); //A banda nesse caso é o número de RBs. Está com 25 RBs (5 MHz)
...
*						itHarq).second.at (harqId).m_rv.at (j)++;
	...
	//Vou ter que pegar o número de RBs disponíveis nos slice.
	int rbgPerTb = (nTbs > 0) ? ((rbgNum - rbgAllocatedNum) / nTbs) : INT_MAX; //Um UE pode ter mais de 1 RBG em seu Transport Block (Tb)
	NS_LOG_INFO(
			this << " Flows to be transmitted " << nflows << " rbgPerTb " << rbgPerTb);
	if (rbgPerTb == 0) {
	...
	do {
		...
		int nLayer = TransmissionModesLayers::TxMode2LayerNum(
				(*itTxMode).second);
		int lcNum = (*itLcRnti).second; //Número de canais lógicos por RNTI
			...
			if (((*it).m_rlcTransmissionQueueSize > 0)
				...
				for (uint8_t j = 0; j < nLayer; j++) {
					...
					UpdateDlRlcBufferInfo((*it).m_rnti,
							newRlcEl.m_logicalChannelIdentity, rlcPduSize);
...
void RrFfMacScheduler::DoSchedUlTriggerReq(
		...
		for (uint16_t i = 0; i < params.m_ulInfoList.size(); i++) {
			if (params.m_ulInfoList.at(i).m_receptionStatus
				...
				uint8_t harqId = (uint8_t) ((*itProcId).second - HARQ_PERIOD)
						% HARQ_PROC_NUM;
				NS_LOG_INFO(
						this << " UL-HARQ retx RNTI " << rnti << " harqId " << (uint16_t)harqId);
				...
				if (free) {
					...
					NS_LOG_INFO(
							this << " Send retx in the same RBGs " << (uint16_t)dci.m_rbStart << " to " << dci.m_rbStart + dci.m_rbLen << " RV " << (*itStat).second.at (harqId) + 1);
				} else {
					NS_LOG_INFO(
	...
	do {
		...
		NS_LOG_INFO(
				this << " RB Allocated " << rbAllocated << " rbPerFlow " << rbPerFlow << " flows " << nflows);
		while ((!allocated)
		...
		NS_LOG_INFO(
				this << " UL Allocation - UE " << (*it).first << " startPRB " << (uint32_t)uldci.m_rbStart << " nPRB " << (uint32_t)uldci.m_rbLen << " CQI " << cqi << " MCS " << (uint32_t)uldci.m_mcs << " TBsize " << uldci.m_tbSize << " harqId " << (uint16_t)harqId);
	...
	for (unsigned int i = 0; i < params.m_macCeList.size(); i++) {
		if (params.m_macCeList.at(i).m_macCeType == MacCeListElement_s::BSR) {
			...
			if (it == m_ceBsrRxed.end()) {
				...
				NS_LOG_INFO(
						this << " Insert RNTI " << rnti << " queue " << buffer);
			} else {
				// update the buffer size value
				(*it).second = buffer;
				NS_LOG_INFO(
						this << " Update RNTI " << rnti << " queue " << buffer);
			}
		}
	}
...
void RrFfMacScheduler::DoSchedUlCqiInfoReq(
	...
	case UlCqi_s::PUSCH: {
		...
		if (itMap == m_allocationMaps.end()) {
			NS_LOG_INFO(
					this << " Does not find info on allocation, size : " << m_allocationMaps.size ());
			return;
		}
...
void RrFfMacScheduler::RefreshDlCqiMaps(void) {
